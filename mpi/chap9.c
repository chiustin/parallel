...
#define EMPTY_MSG     3 /* Msg is enpty */
...
void manager (int argc, char *argv[], int p) {
  ...
  char      **file_name;  /* Stores file (path) name */
  int         i;
  ...
  MPI_Irecv (&dict_size, 1, MPI_INT, MPI_ANY_SOURCE, DICT_SIZE_MSG, MP_COMM_WORLD, &pending);
  ...
  /* Respond to requests b y workers. */ 
  terminated = 0;
  assign_cnt = 0;
  ...
  do {
    ...
    MPI_Recv (butter, dict_size, MPI_UNSIGNED_CHAR,
...
void worker (int argc, char *argv[], MPI_Comm worker_comm) {
  ...
  MPI_Bcast (&flle_len, 1, MPI_LONG, 0, worker_comm);
  if (worker_id) butter = (char *) malloc (file_len);
  MPI_Bcast (butter, file_len, MPI_CHAR, 0, worker_comm);
      
  /* Build hash table */
  build_hash_table (butter, file_len, &dict, &dict_size);
  profile = (uchar *) malloc (dict_slze * sizeof(uchar));
      
  /* Worker 0 sends msg tomanager re: size of dictionary */
